...
public class WLPriceChecker {
	...
	/**
	 * Scans all the entries in the adapter and updates
	 * prices if the price information has changed
	 */
	public void priceCheck() {
		//Open up the SQLite database
		mDbHelper = new WLDbAdapter(mAdapter.getContext().getApplicationContext());
		mDbHelper.open();
	...
	private class WLPriceCheck extends AsyncTask<String, Integer, String> {
		...
		protected void onProgressUpdate(Integer... values) {
			...
			mDbHelper.updateEntry(mAdapter.getItem(values[0]).getId(), mAdapter.getItem(values[0]));
		}
